...
class SingleVideo2MOS(Vid2MOS):
    # one batch: torch.Size([bs, 3, 540, 960])
    # batch of frames (clip with clip_size=1)
    # vid2mos one batch: torch.Size([bs, 6, 3, 540, 960])
    """TODO: skip frame """
    """may include roi
    ...
    def get_input_blocks(self):
        ...
        return ImageBlock if self.clip_size==1\
         else partial(SingleClipBlock, clip_size=self.clip_size),
         # please note we don't use multi clip here. (to use multi clip, we need crop to the same size )
         # partial(ClipBlock, clip_size=self.clip_size),
    ...
    def get_df(self):
        if self._df is None:
            ...
            if self.clip_num is None:
                ...
                # TODO: better unify these
                if self.clip_size==1: # for 2d case, we need .jpg (ImageBlock)
                  files = [f'image_{x+self.clip_size:05d}.jpg' for x in range(0, num_frame-self.clip_size+1, self.clip_size)] # drop last clip
                else: # for 3d case, we don't need .jpg (MultiClipBlock)
                  files = [f'image_{x+self.clip_size:05d}' for x in range(0, num_frame-self.clip_size+1, self.clip_size)] # drop last clip
            else:
                ...
                files = [f'image_{x:05d}' for x in last_frame_index ]
