...
from ._blend import blend_output

class QualityMap:
    """Note here the image is fastai image"""
    interpolation = 'bilinear'
    def __init__(self, mat, img, global_score=0, **kwargs):
    ...
    def plot(self, vmin=0, vmax=100):
        ...
        # fastai image
        self.img.show(ctx=axes[0], title='Input image')  # title mos
        self.img.show(ctx=axes[1], title=f'Predicted: {self.global_score:.2f}')  # title prediction

        # fastai1 _, H, W = self.img.shape
        H, W = self.img.shape
        h, w = self.mat.shape  # self.mat.size()
        extent = (0, W // w * w, H // h * h, 0)

        axes[1].imshow(self.mat, alpha=0.8, cmap='magma',
                       extent=extent, interpolation=self.interpolation)
    ...
    def blend(self, mos_range=(0, 100), alpha=0.8, resample=Image.BILINEAR):
        ...
        return blend_output(self.pil_image, self.mat)
...
def predict_quality_map(self, sample, blk_size=None):
    ...
    # self.dls.device
    self.model.input_block_rois(blk_size, [sample.shape[-2], sample.shape[-1]], device=sample.device)  # self.dls.img_raw_size
