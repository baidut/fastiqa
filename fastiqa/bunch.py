...
"""
remove abbr - if self._abbr is None: self.abbr = self.__name__.split('__')[0]
...
class IqaDataBunch():
    ...
    def __lshift__(self, other): # dls = Im2MOS(bs=2) << KonViD
    ...
    def bunch(self, x, **kwargs):
        ...
        if isinstance(x, (self.__class__, IqaDataBunch) ):
            cfg = x.__dict__
        elif type(x) == str: # path to json file
            cfg = load_json(x)
        elif type(x) == dict:
            cfg = x
        else:
        ...
        d = self.__dict__.copy()  # not all attributes
        # self._bunching = True
        d.update(cfg)
        d.update(kwargs)
